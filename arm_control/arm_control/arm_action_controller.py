...
from agribot_interfaces.action import CropInspection
...
class ArmControllerNode(Node):
    def __init__(self):
        ...
        # --- CALIBRATED ESP32-CAM VISION PARAMETERS ---
        self.lower_green = np.array([39, 63, 21])
        self.upper_green = np.array([119, 255, 255])
        self.minimum_blob_area = 2000 # Minimum pixels to be considered a valid crop
        ...
        self.action_server = ActionServer(self, CropInspection, '/arm/inspect_crop', self.execute_callback, callback_group=self.cb_group)
        ...
        for attempt in range(3):
            try:
                for idx in motor_mapping[motor]:
                    ...
                    send_angle = max(0, min(180, send_angle)) 
    ...
    def camera_callback(self, msg):
        ...
        if self.is_centering:
            ...
            # Clean up tiny noise (small weeds/reflections)
            ...
            if contours:
                ...
                # Ensure the blob is large enough to be considered a crop!
                if cv2.contourArea(largest_contour) > self.minimum_blob_area: 
                    ...
                    self.blob_detected = False # Blob too small
            else:
                self.blob_detected = False # No green found
    ...
    def align_to_target(self, publish_feedback, x_offset=0, y_offset=0):
        ...
        # Max 40 attempts (~2 seconds) to find a blob and align
        while self.is_centering and centering_attempts < 40: 
        ...
        self.is_centering = False
        time.sleep(0.5) 
        return self.blob_detected # Returns True if successful, False if it timed out looking at dirt

    def execute_callback(self, goal_handle):
        self.get_logger().info('Executing goal...')
        feedback_msg = CropInspection.Feedback()
        
        # MODIFIED: Also publish to /arm/status
        def publish_feedback(msg_text):
            ...
            # 🔥 publish to topic for mobile app
            self.publish_status(msg_text)

        self.is_inspecting = True 
        self.set_flashlight(200)
        time.sleep(1.0) # Let camera adjust to light
        
        publish_feedback("Moving to Look Position...")
        self.move_simultaneously(self.look_pos, speed_delay=0.015)
        
        # Give the camera a moment to acquire the first frames
        time.sleep(1.0)
        
        # Check if we actually see a crop. If not, abort the mission!
        blob_found = self.align_to_target(publish_feedback, x_offset=0, y_offset=0)
        
        if not blob_found:
            publish_feedback("No crop of sufficient size detected. Skipping inspection.")
            self.move_simultaneously(self.home_pos)
            self.set_flashlight(0)
            self.is_inspecting = False
            
            # Succeed the goal so the Nav node happily resumes driving
            goal_handle.succeed()
            result = CropInspection.Result()
            result.success = True
            
            # ADDED: Publish final result before return
            self.publish_status("Skipped: No crop detected.")
            
            result.message = "Skipped: No crop detected."
            return result
        ...
        # ADDED: Publish final result before return
        self.publish_status("Routine Completed Successfully!")
